...
export default function Sidebar({ isOpen }) {
    ...
    return (
        <aside className={`${isOpen ? 'w-64' : 'w-20'} bg-slate-900 text-white transition-all duration-300 ease-in-out flex flex-col`}>
            {/* Logo Area */}
            <div className="h-16 flex items-center justify-center border-b border-slate-800">
                <h1 className={`font-bold text-xl ${!isOpen && 'hidden'}`}>
                    SIPERAK<span className="text-blue-500">.</span>
                </h1>
                {/* Icon Logo kalau sidebar tertutup */}
            ...
            <nav className="flex-1 py-6 space-y-2 px-3">
                ...
                ${isActive
                        ...
                        >
                            <Icon size={20} />
                            <span className={`ml-3 font-medium whitespace-nowrap ${!isOpen && 'hidden'}`}>
                                {item.label}
                            </span>

                            {/* Tooltip kalau sidebar closed */}
                            {!isOpen && (
            ...
            <div className="p-4 border-t border-slate-800">
                <button className="flex items-center w-full px-4 py-2 text-slate-400 hover:text-red-400 hover:bg-slate-800 rounded-lg transition-colors">
                    <LogOut size={20} />
